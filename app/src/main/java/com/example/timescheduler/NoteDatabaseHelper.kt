...
class NoteDatabaseHelper(context: Context):SQLiteOpenHelper(context, DATABASE_NAME,null,
    ...
    fun insert(noter: Noter) {
        val db = writableDatabase
        val values = ContentValues().apply {
            put(COLUMN_TITLE, noter.title)
            put(COLUMN_CONTENT, noter.content)
    ...
    fun getallNotes(): List<Noter> {
        val notesList = mutableListOf<Noter>()
        ...
        while (cursor.moveToNext()) {
            ...
            val note = Noter(id, title, content)
            notesList.add(note)
        }
        cursor.close()
        db.close()
